...
function countPathsForSum(root, sum) {
...
function countPathsRecursive(currentNode, sum, currentPath) {
  ...
  for (let i = currentPath.length - 1; i >= 0; i--) {
    ...
    //if the sum of the sub-path is === sum, increment pathcount
  ...
  //need to remove current node while going up the recursive call stack
  currentPath.pop();
  return pathCount;
}
...
console.log(`Tree has paths: ${countPathsForSum(root, 11)}`);
